...
fn evaluate(
    ...
    h: f64,
    target: f64,
    prob_fn: fn() -> InitialValueSystemProblem<Function<(f64, Vec<f64>)>>,
    exact: Vec<Point2D>,
) {
    ...
    println!(
        "SSE for {} with classic ERK method: {:e}",
        name,
        sse(&exact, &to_plot_classic)
    );
    println!(
        "MSE for {} with classic ERK method: {:e}\n",
        name,
        sse(&exact, &to_plot_classic) / exact.len() as f64
    );
    ...
    println!(
        "SSE for {} with England ERK method: {:e}",
        name,
        sse(&exact, &to_plot_england)
    );
    println!(
        "MSE for {} with England ERK method: {:e}\n",
        name,
        sse(&exact, &to_plot_england) / exact.len() as f64
    );
    ...
    println!(
        "SSE for {} with 3/8 ERK method: {:e}",
        name,
        sse(&exact, &to_plot_three_eight)
    );
    println!(
        "MSE for {} with 3/8 ERK method: {:e}\n\n\n",
        name,
        sse(&exact, &to_plot_three_eight) / exact.len() as f64
    );
    ...
    plot_line_points_on(
        axis,
        &to_plot_classic,
        &[Caption(&format!("Classic ERK with h={}", h)), Color("red")],
    );
    ...
    plot_line_points_on(
        ...
        &to_plot_england,
        &[
            Caption(&format!("England ERK with h={}", h)),
            Color("green"),
        ],
    );
    ...
    plot_line_points_on(
        axis,
        &to_plot_three_eight,
        &[Caption(&format!("3/8 ERK with h={}", h)), Color("blue")],
    );
